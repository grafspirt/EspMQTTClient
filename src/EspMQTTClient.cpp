...
void EspMQTTClient::loop() {
  ...
  if ( WiFi.status() == WL_CONNECTED ) {
    ...
    if ( mMqttClient->connected() ) {
      ...
      if ( currentMillis - mLastMqttConnectionMillis > MQTT_CONNECTION_RETRY_DELAY || mLastMqttConnectionMillis == 0 ) {
        connectToMqttBroker();
      }
    }
...
void EspMQTTClient::subscribe(const String &topic, MessageReceivedCallback messageReceivedCallback) {
  ...
  if ( mTopicSubscriptionListSize >= MQTT_MAX_TOPIC_SUBSCRIPTION_LIST_SIZE ) {
    if ( mEnableSerialLogs ) {
...
void EspMQTTClient::unsubscribe(const String &topic) {
  ...
  for ( int i = 0; i < mTopicSubscriptionListSize; i++ ) {
    if ( !found ) {
      if ( found = mTopicSubscriptionList[i].topic.equals(topic) ) {
    ...
    if ( found ) {
      if ( (i + 1) < mTopicSubscriptionListSize ) {
...
void EspMQTTClient::mqttMessageReceivedCallback(char* topic, byte* payload, unsigned int length) {
  ...
  char buffer[MQTT_MAX_PAYLOAD_SIZE];

  if ( length >= MQTT_MAX_PAYLOAD_SIZE ) {
    length = MQTT_MAX_PAYLOAD_SIZE - 1;
  }
  ...
  // Handle the message by subscriber
  bool found = false;
  for ( int i = 0; i < mTopicSubscriptionListSize && !found; i++ ) {
    if ( found = mTopicSubscriptionList[i].topic.equals(topic) ) {
